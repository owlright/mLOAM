...
CloudT::Ptr GenerateMapCloud() {
    ...
    for (int i = 0; i < 800; ++i) {
        cloud->push_back(
            PointT((rand() % 100) / 10.0, (rand() % 100) / 10.0, noise(eng)));
    }

    // Wall: x ≈ 5
    for (int i = 0; i < 800; ++i) {
        cloud->push_back(PointT(5.0 + noise(eng), (rand() % 100) / 10.0,
                                (rand() % 50) / 10.0));
    }
...
// =====================================================
CloudT::Ptr TransformCloud(const CloudT::Ptr& cloud,
    ...
    for (const auto& p : cloud->points) {
        ...
        out->push_back(PointT(pt_t.x(), pt_t.y(), pt_t.z()));
    }
    return out;
}
...
// =====================================================
bool FitPlane(const std::vector<Eigen::Vector3d>& pts, Eigen::Vector3d& center,
...
struct PointPlaneResidual {
    PointPlaneResidual(const Eigen::Vector3d& curr,
    ...
    bool operator()(const T* q, const T* t, T* residual) const {
        ...
        Eigen::Matrix<T, 3, 1> p = quat * curr_.cast<T>() + trans;

        residual[0] = normal_.cast<T>().dot(p - plane_pt_.cast<T>());
    ...
    static ceres::CostFunction* Create(const Eigen::Vector3d& curr,
                                       const Eigen::Vector3d& plane_pt,
                                       const Eigen::Vector3d& normal) {
        return new ceres::AutoDiffCostFunction<PointPlaneResidual, 1, 4, 3>(
...
int main() {
    ...
    Eigen::Quaterniond q_gt(Eigen::AngleAxisd(0.25, Eigen::Vector3d::UnitZ()));
    Eigen::Vector3d t_gt(0.6, 0.3, 0.2);

    CloudT::Ptr scan_cloud = TransformCloud(map_cloud, q_gt, t_gt);
    ...
    for (const auto& p : scan_cloud->points) {
        if (kdtree.nearestKSearch(p, 5, indices, distances) < 5) continue;
        ...
        if (!FitPlane(neighbors, center, normal)) continue;

        Eigen::Vector3d curr(p.x, p.y, p.z);
        problem.AddResidualBlock(
            PointPlaneResidual::Create(curr, center, normal), nullptr, q, t);
    }
    ...
    std::cout << "\nGround truth translation : " << t_gt.transpose()
              << "\nEstimated translation    : " << t_est.transpose()
              << std::endl;

    // -------------------------------------------------
    // 6. Visualization
    // -------------------------------------------------
    CloudT::Ptr aligned_cloud = TransformCloud(scan_cloud, q_est, t_est);
    ...
    viewer.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0.0, 1.0, 0.0, "map");
    viewer.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 0.0, 0.0, "scan");
    viewer.setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0.0, 0.0, 1.0, "aligned");
